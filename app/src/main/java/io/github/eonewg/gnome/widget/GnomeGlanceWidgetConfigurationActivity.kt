...
@AndroidEntryPoint
class GnomeGlanceWidgetConfigurationActivity : FragmentActivity() {
    ...
    fun ConfigurationScreen() {
        ...
        LaunchedEffect(Unit) {
            try {
                ...
                val glanceId = GlanceAppWidgetManager(this@GnomeGlanceWidgetConfigurationActivity)
                    .getGlanceIdBy(appWidgetId)
                val prefs = getAppWidgetState(
                    context = this@GnomeGlanceWidgetConfigurationActivity,
                    definition = PreferencesGlanceStateDefinition,
                    glanceId = glanceId
                )
                
                selectedTag = prefs[GnomeWidgetKeys.filterTag]
                pinnedOnly = prefs[GnomeWidgetKeys.pinnedOnly] ?: false
                maxItems = (prefs[GnomeWidgetKeys.maxItems] ?: 10).toFloat()
            } catch (e: Exception) {
                // Ignore error
            } finally {
                isLoading = false
    ...
    private fun saveConfig(filterTag: String?, pinnedOnly: Boolean, maxItems: Int) {
        lifecycleScope.launch {
            ...
            updateAppWidgetState(this@GnomeGlanceWidgetConfigurationActivity, glanceId) { prefs ->
                if (filterTag != null) {
                    prefs[GnomeWidgetKeys.filterTag] = filterTag
                } else {
                    prefs.remove(GnomeWidgetKeys.filterTag)
                }
                prefs[GnomeWidgetKeys.pinnedOnly] = pinnedOnly
                prefs[GnomeWidgetKeys.maxItems] = maxItems
            }
            
            GnomeGlanceWidget().update(this@GnomeGlanceWidgetConfigurationActivity, glanceId)
